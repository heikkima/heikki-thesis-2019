...
ovat jaettuja kokonaisuuksia sovelluksesta. Jokaisen mikropalvelun tulee olla
itsenäisesti hallittavissa ja muutettavissa. Paketoidusta mikropalvelusta
voidaan käynnistää kontti (container) missä tahansa käyttöjärjestelmässä, kunhan
tämä tukee Docker teknologiaa. Kontit ovat hyvin kevyitä ja yhdessä
isäntäkoneessa (host) voidaan ajaa samanaikaisesti useita kontteja. Teknologian
etuna on sovellusten yksinkertainen pakkaaminen ja monistaminen erilaisiin
ympäristöihin. Tämä nopeuttaa sovelluksen julkaisemista tuotantoon, ja
tuotannossa tämän skaalaamista (Docker, s.a.).
...
*AWS ECS* on Amazonin pilvipalvelualustalla toimiva konttiorkestrointi työkalu.
...
konsolin tai AWS rajapinnan avulla. AWS ECS skaalaa ylöspäin/alaspäin
automaattisesti kontteja tarvittaessa. Automaattiskaalaus noudattaa
skaalaamisessa ennalta määrättyjä sääntöjä, jotka määrittelevät vähimmäis- ja
enimmäismäärän konteille. Mikäli resurssit pilvi-instanssilta voidaan
instanssien määrää myös skaalata ylöspäin tai alaspäin. AWS ECS on saatavilla
vain Amazon Web Services ympäristössä (Sarkar & Shah. 2018)*. Docker Swarm* on
Dockerin kehittämä orkestrointi työkalu Docker konteille. Docker Swarm ryhmittää
konttien isäntäkoneet yhteen varantoon ja tarjoaa käyttäjälle työkalut hallita
varantoa yhtenäisesti (Ravindra, 2018; Paraiso, Challita, Al-Dhuraibi & Merle
2016).
...
isoimmista avoimenlähdekoodin yhteisöistä. Yhteisö on ollut aktiivisesti mukana
...
Amazon Elastic Container Service for Kubernetes (AWS EKS). Ja koska K8s on
julkaistu avoimena lähdekoodina, pystyvät pilvipalvelun tarjoajat luomaan omalle
K8s palveluilleen lisäominaisuuksia ja liittämään K8s osaksi heidän
ekosysteemiään. Tämän lisäksi he voivat myös käyttää olemassa olevia työkaluja,
joita K8s yhteisö on luonut ja jakanut.

Kuberneteksen arkkitehtuuri
---------------------------

K8s koostuu itsessään myös klusterista. Klusteri muodostuu monesta koneesta
(fyysisestä tai virtuaalisesta), joissa ajetaan K8s ohjelmaa. Koneita voidaan
liittää jälkikäteen järjestelmään helposti, mikäli järjestelmän resurssit eivät
riitä. Klusterin sisällä on yksi tai useampia mestarisolmuja (master node) ja
useita kätyrisolmuja (minion node) (Kubernetes 2019a). Tässä kappaleessa käymme
läpi muutamaa isompaa komponenttia K8s:n klusterissa. Näiden lisäksi K8s
sisältää pienempiä mutta silti tärkeitä komponentteja.
...
myös kyseistä rajapintaa, kun kommunikoivat mestarisolmun kanssa. *Etcd* on
klusterin käytössä oleva tietokanta. Klusterin rajapinta, Kube-apiserver,
tallentaa datan avain-arvo (key-value) pareina tietokantaan. Tietokantaan
tallennetaan muun muassa klusterin nykyinen tila ja haluttu tila. K8s klusteri
käyttää etcd tietokantaa totuuden lähteenä (source of truth) ja on tärkeää luoda
varmuuskopioita tästä. K8s dokumentaatio suosittelekin luomaan etcd klusterin ja
luomaan kyseisen komponentin usealle solmulle. *Kube-scheduler* tarkkailee, jos
uusia kapseleita (pod. Komponentti, joka pitää sisällään kontteja) luodaan ja
ohjaa nämä oikeisiin kätyrisolmuihin. Kube-scheduler osaa ottaa huomioon
kapselin resurssi- ja muut vaatimukset, kun valitsee tälle sopivaa kätyrisolmua.
...
monta, kun rakennetaan aina käytettävissä (highly available, lyhenne HA) olevaa
...
ottavat mestariin yhteyttä kuormantasaajan (loadbalancer) kautta (kuva 1).
Kuormantasaaja ohjaa liikenteen mestarille, jonka tämä tunnistaa olevan pystyssä
(Kubernetes 2019b; Arundel & Domingus, 2019).

![](media/af912b12fe44e876006e1b882f22868a.png)

Kuva 1. HA Kubernetes klusteri (mukailen Kubernetes 2019a)

### Pod

Kapseli (pod) muodostuu yhdestä tai useammasta hyvin tiiviiksi kytketyistä
konteista. Mikropalvelut voidaan yleensä eriyttää omiin kontteihinsa ja kontti
rakentaa yksin kapselin sisälle. On kuitenkin mahdollista, että palvelut ovat
luotu hyvin riippuvaisiksi toisistaan. Tällöin palvelut pakataan omiin
kontteihinsa, mutta keskenään riippuvaiset kontit rakennetaan saman kapselin
sisälle. Kapseli sijoitetaan aina kätyrisolmuun. Tämän sisällä olevat kontit
jakavat saman IP osoitteen ja muistin. Jokaisella kapselilla on oma
elämänkaarensa. Elämänkaaren eri vaiheita ovat:
...
-   Käynnissä (Running)
    ...
    -   Kapseli on ohjattu kätyrisolmulle ja kontti kapselin sisälle luotu.
...
2). Palvelu-objektissa on sisäänrakennettu kuormantasaaja, joka lähettää
yhteyden vain saatavilla oleville kapseleille

![](media/8b02216fabb9bd0072ad3bc06a1efad3.png)

Kuva 2. Palvelu-objekti ClusterIP

*NodePort* tyyppinen palvelu-objekti näkyy K8s klusterin ulkopuolelle.
Palvelu-objekti avaa jokaisen klusterin kätyrisolmusta saman portin (kuva 3).
...
portti, jonka liikenne ohjataan eteenpäin palvelulle (kuva 3). Kuormantasaaja
...
useampi portti määritelty eri palveluille. (Ku

![](media/e39064b34f809f80c22eee2f5cd85781.png)

![](media/b4d791ba38ebc95ab79af7ed10e7dfb3.png)

bernetes, 2018c; Kubernetes, 2018d; Yegulalp, 2019; Sandeep, 2018).

Kuva 3. Vasemalla palvelu-objekti NodePort. Oikealla palvelu-objekti
LoadBalancer

### Minion node

Kätyrisolmut ovat koneita, joissa sijaitsee sovelluksen kapselit. Kätyrisolmut
...
tullut kutsu tälle tarkoitetulle kapselille (kuva 4). Kube-proxy tarkkailee
...
sääntöjä IP-paketeille ja NAT sääntöjä) (kuva 4). Tällä tavoin kube-proxy ohjaa
...
Kuva 4. Vasemmalla: välitys tila: userspace; Oikealla: välitystila: ip-tables
...
konfiguraatiotiedostoon. Tiedoston kirjoittajan ei tarvitse kirjoittaessa tietää
ympäristön tilannetta, jotta pääsisi haluttuun tilaan. Vastaavasti jos samaan
lopputulokseen halutaan päästä käyttämällä komentoja imperatiivisesti
(imperative). Tulisi komentojen kirjoittaja tietää missä tilassa ympäristö on
sillä hetkellä. Ympäristön nykyisestä tilasta voidaan päätellä, tuleeko
...
controller). Kyseinen valvoja skannaa tietyin väliajoin ympäristössä päällä
...
Kun sovellus siirtyy tuotantoon, tulee sovelluksen pystymään skaalautumaan
nopeasti ja huomaamattomasti sovelluksen käyttäjille. Suorituskyvyn
pullonkaulana voi olla esimerkiksi yhden mikropalvelun kontti. Konttien
lukumäärää voidaan K8s:sa lisätä muuttamalla kapselien määrää
konfiguraatiotiedossa. Tämän voi kuitenkin myös tehdä automaattisesti K8s:sa
automaattiskaalauksella (autoscaling). Skaalaaja kykenee automaattisesti
skaalaamaan kapseleita horisontaalisesti annettujen sääntöjen puitteissa.
Automaattiskaalaaja ei itse luo tai poista kopioita kapseleista. Vaan tämä
kommunikoi replikaatio valvojan kanssa, joka sitten luo tai poistaa kapseleita
(kuva 5). Näin kapseleiden skaalaus ei luo konflikteja replikaatio valvojan
kanssa. Uudet kapselit käynnistetään vierekkäin jakaen sovelluksen kuormaa.
Automaattiskaalaajalle voi määrittää metriikkaa, jonka perusteella tämä skaalaa
kapseleita ylös tai alaspäin. Metriikka voi olla esimerkiksi prosessorin
käyttöprosentti tai joku sovelluksen tarjoama oma metriikka.

![](media/5248df1123cd43b4dd4671591fefa487.png)

Kuva 5. Automaattiskaalaaja (mukailen Sayfan, 2017)
...
palveluntarjoajilla on mahdollista automaattisesti skaalata K8s klusteria. Jos
palvelu tunnistaa mestarisolmun yrittävän luoda uuden kapselin, mutta resurssit
eivät riitä tai eivät vastaa kapselin vaatimuksia. Palvelu liittää uuden koneen
K8s klusteriin ennalta määrättyjen sääntöjen mukaisesti. Vastaavasti palvelut
...
K8s ei ole riippuvainen alla olevastaan infrastruktuuristaan. Tämä on nähty
yhtenä K8s:n suurimmista hyödyistä. K8s:n voi pystyttää omaan konesaliin,
...
kanssa (kuva 6). AWS takaa pilvipalveluntarjoajana laitteiston, ohjelmiston,
verkkoyhteyden toimivuuden ja fyysisen turvauksen konesaleilla. Edellä
mainittujen lisäksi AWS huolehtii EKS palvelussa K8s klusterin ohjaamon (mestari
kätyrit) konfiguraation ja toimivuuden sekä etcd -tietokannan saatavuuden.
Mestareiden ja tietokannan saatavuuden takaamiseksi AWS ajaa mestarit usealla
saatavuus alueilla (availability zones, fyysesti eri paikoissa olevia
konesaleja). Tämän lisäksi AWS tarkkailee jatkuvasti mestarisolmuja ja korvaa
vialliset solmut uusilla. Asiakkaan vastuulla on K8s klusterin muu
konfiguraatiot, esimerkiksi kätyrisolmun konfiguraatio, ohjaamon yhteys
asiakkaan VPC verkkoon (virtual private cloud, asiakkaan oma verkko pilvessä).
...
Kuva 6. EKS jaettu vastuu (mukailen Amazon Web Services, 2018a)

EKS toiminta AWS ympäristössä
-----------------------------

EKS palvelu on suunniteltu tukemaan useita AWS:n olemassa olevia palveluita,
parantaakseen EKS palvelun skaalautumista ja tietoturvaa. Näitä palveluita ovat
Elastic Load Balancing, IAM, VPC ja PrivateLink.

*Elastic Load Balancing* (ELB) on AWS:n tarjoama kuormantasaaja palvelu. EKS
tukee kahden tyyppistä ELB kuormantasaajaa, Network Load Balancer (NLB) ja
Classic Load Balancer (CLB). Oletus kuormantasaaja tyyppi on CLB. NLB jakaa
...
tason ohjauksen ansiosta NLB on erittäin nopea ja tehokas. Tämä kuormantasaaja
malli on vielä alpha vaiheessa, eikä suositella vielä tuotanto K8s klustereihin
(Kubernetes, 2018c). CLB on yksi tuetuista kuormantasaajista palvelu-objekti
tyypissä LoadBalancer (otsikko 2.2.3). (Bala 1.12.2017)

*IAM* on AWS alustalla identiteetin ja pääsynhallinta -palvelu. Palvelun ideana
on vastata kysymykseen: Kenellä on oikeus tehdä mitä ja missä? IAM palvelussa
määritellään rooleja, käyttäjiä ja ryhmiä. Näihin objekteihin liitetään
menettelytapa (policy), jossa määritellään oikeuksista. EKS palvelu luo
käyttäjän puolesta resursseja, joita hyödynnetään K8s klusterissa. EKS tarvitsee
oikeudet näiden resurssien luontiin. AWS kertoo dokumentaatiossaan
vähimmäisoikeudet EKS palvelulle (Amazon Web Services, s.a. (a)).

*VPC* (Virtual Private Cloud) on ympäristö johon käyttäjä voi luoda AWS
resurssejaan virtuaaliverkkoon. VPC:n sisälle luodaan pienempiä verkkoja
aliverkkoja (subnet). Aliverkot määritellään saatavuus alueilla ja voivat olla
julkisia tai suljettuja verkkoja. EKS vaatii luonnin klusterin yhteydessä
vähintään kaksi aliverkkoa eri saatavuusalueilta. AWS suosittelee
kuormantasaajan julkiselle aliverkolle ja kätyrisolmut suljetulle aliverkolle
(kuva 7). AWS luo klusterin luonnin yhteydessä asiakkaan aliverkoille Elastic
Network Interface (ENI) resurssin, mikäli klusterin oikeudet tämän sallii.
Klusterin ohjaamo käyttää ENI -resurssia kommunikoidakseen kätyreiden kanssa.
Klusterin ylläpitäjä ja kätyrisolmut ottavat yhteyttä klusterin ohjaamon API
palvelimelle kuormantasaajan kautta. Ohjaamon API palvelin voidaan muuttaa myös
yksityiseksi, tällöin yhteydet API palvelimeen menevät ENI resurssin kautta.
Tämä vaatii ylläpitäjää luomaan VPC:n sisälle bastion -instanssin
(virtuaalikone, joka näkyy ulkoverkkoon ja jolla on pääsy sisäverkkoon), jonka
kautta ylläpitäjä voi ottaa yhteyttä klusterin ohjaamoon.

![](media/de36c1045505160d40a5f390d5081088.png)

Kuva 7. AWS:n suosittelema verkko infrastruktuuri EKS palvelulle (Amazon Web
Services, s.a (b))
...
hoitaa mestarisolmujen ja kätyrisolmujen tietoturvapäivityksistä. AKS, GKE ja
...
kätyrisolmuja. Klusterille luodaan yksi tai useampi kätyrisolmu varanto (minion
node pool). Samanlaisia ominaisuuksia löytyy myös GKE ja EKS palveluissa. Kaikki
kätyrisolmut oman varannon sisällä ovat identtisiä. DOK8s poistaa tarvittaessa
...
vikasietoisuutta, esimerkiksi paikallisen sähkökatkos. DOK8s klustereissa
mestarisolmuja on vain yksi kappale (DigitalOcean, 2018).
...
EKS on hyvä palvelu, mikäli haluaa paljon hallintaa omassa ympäristössään. EKS
tarjoaa
